...
a <- Resumen %>% group_by(year) %>% summarize(means = mean(Emissions, na.rm = TRUE))
# Now we choose a very nice palette 
library(RColorBrewer)
p <-barplot(a$means, col = brewer.pal(length(a$means), "Spectral"), names.arg = a$year, 
		xlab = '', ylab = '', main = 'Total PM2.5 emissions all sources have been decreased', axes = FALSE)
