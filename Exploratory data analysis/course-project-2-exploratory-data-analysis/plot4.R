...
codes <-  Codes$SCC[grep("Coal",Codes$EI.Sector)] # Fuel - Comb, Coal 
codes <- unique(codes)
a <- subset(Resumen, SCC %in%codes)

# We plot to show the that emissions in U.S. from 1998-2008 have been decreased
...
ggplot(data = a, aes(x = factor(year(year)), y = Emissions)) + geom_bar(fill = brewer.pal(length(a$year), "PRGn"),
	...
	xlab("year") +ylab("Emissions (tons)")+ theme_bw() +  
		ggtitle('Emissions of PM2.5 in U.S.') + guides(fill=FALSE) 
